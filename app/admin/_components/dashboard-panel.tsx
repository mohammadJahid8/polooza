...
export default function DashboardPanel({ rsvps, onRefresh }: DashboardPanelProps) {
  /* Compute stats */
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        <div className="text-center p-[1.2rem]" style={{ border: "1px solid rgba(200, 168, 75, .2)" }}>
          <div className="font-[family-name:var(--font-cinzel)] text-[2rem] text-palooza-gold">{rsvps.length}</div>
          <div className="text-[0.6rem] tracking-[0.18em] uppercase text-palooza-sand mt-[0.2rem]">RSVPs received</div>
        </div>
        <div className="text-center p-[1.2rem]" style={{ border: "1px solid rgba(200, 168, 75, .2)" }}>
          <div className="font-[family-name:var(--font-cinzel)] text-[2rem] text-palooza-gold">{guestsWithAllergies}</div>
          <div className="text-[0.6rem] tracking-[0.18em] uppercase text-palooza-sand mt-[0.2rem]">With dietary needs</div>
        </div>
        <div className="text-center p-[1.2rem]" style={{ border: "1px solid rgba(200, 168, 75, .2)" }}>
          <div className="font-[family-name:var(--font-cinzel)] text-[2rem] text-palooza-gold">{totalYes}</div>
          <div className="text-[0.6rem] tracking-[0.18em] uppercase text-palooza-sand mt-[0.2rem]">Event confirmations</div>
        </div>
      </div>
      ...
      <table className="w-full border-collapse text-[0.8rem]">
        <thead>
          <tr>
            <th className="text-left py-2 px-3 text-[0.6rem] tracking-[0.18em] uppercase text-palooza-gold font-normal" style={{ borderBottom: "1px solid rgba(200, 168, 75, .2)" }}>Event</th>
            <th className="text-left py-2 px-3 text-[0.6rem] tracking-[0.18em] uppercase text-palooza-gold font-normal" style={{ borderBottom: "1px solid rgba(200, 168, 75, .2)" }}>✓ Yes</th>
            <th className="text-left py-2 px-3 text-[0.6rem] tracking-[0.18em] uppercase text-palooza-gold font-normal" style={{ borderBottom: "1px solid rgba(200, 168, 75, .2)" }}>✕ No</th>
            <th className="text-left py-2 px-3 text-[0.6rem] tracking-[0.18em] uppercase text-palooza-gold font-normal" style={{ borderBottom: "1px solid rgba(200, 168, 75, .2)" }}>Pending</th>
          </tr>
        </thead>
        <tbody>
          {EVENTS.map((ev) => {
            const yes = rsvps.filter((r) => r.rsvp && r.rsvp[ev.key] === "yes").length;
            const no = rsvps.filter((r) => r.rsvp && r.rsvp[ev.key] === "no").length;
            const pending = rsvps.length - yes - no;
            return (
              <tr key={ev.key}>
                <td className="py-2 px-3 text-palooza-ivory" style={{ borderBottom: "1px solid rgba(255,255,255,.04)" }}>{ev.label}</td>
                <td className="py-2 px-3 text-[#5dba7d] font-medium" style={{ borderBottom: "1px solid rgba(255,255,255,.04)" }}>{yes}</td>
                <td className="py-2 px-3 text-[#e07070]" style={{ borderBottom: "1px solid rgba(255,255,255,.04)" }}>{no}</td>
                <td className="py-2 px-3 text-palooza-sand" style={{ borderBottom: "1px solid rgba(255,255,255,.04)" }}>{pending > 0 ? pending : "—"}</td>
              </tr>
            );
          })}
        </tbody>
        ...
        {Object.keys(allergyCounts).length === 0 ? (
          ...
          Object.entries(allergyCounts)
            ...
            .map(([name, count]) => (
              ...
              >
                <div className="font-[family-name:var(--font-cinzel)] text-[1.1rem] text-palooza-gold">{count}</div>
        ...
        {rsvps.length === 0 ? (
          ...
          rsvps.map((r, i) => {
            ...
            return (
              ...
              >
                <div className="font-[family-name:var(--font-cinzel)] text-base text-palooza-ivory mb-[0.3rem]">
                  {r.name || "Unknown"}
                </div>
                ...
                <div className="mt-2">
                  {allergies.length > 0 ? (
                    ...
                    <span className="text-palooza-ivory/30 text-[0.75rem] italic">None</span>
                  )}
                  {r.other && (
